...
**Lesson 87 — Never Let an Accept Watcher Be Permanently Lost**
The `StreamServer` accept path arms a `WaitReadable` (io_uring `POLL_ADD`) on the listening socket. After each accepted connection the watcher is re-armed in `accept_callback`'s `defer`. The stock code did two things that permanently kill a busy proxy after it has been idle for hours:
1. The re-arm was `enqueue_accept(server) catch {}` — **any** re-arm failure (transient SQE-ring exhaustion, blocking-task pool exhaustion) was silently swallowed, so the listening socket lost its watcher with no recovery.
2. On `EMFILE`/`ENFILE` (per-process / system-wide fd exhaustion) it set `accept_paused = true` and returned without scheduling any retry. `accept_paused` is only cleared in `start_serving()`, which asyncio never calls again — so the listener is dead forever.
- **Why "after idle":** while connections are flowing, other in-flight SQEs (`reserved_slots > 0`) keep the loop alive and mask the missing accept watcher. Once the proxy idles and all connections close, the accept watcher is the *only* in-flight op; if it is lost, `reserved_slots` can hit 0 and the loop can never be woken by a new connection → it looks exactly like "works fine, then freezes after hours."
- **Fix:** On any re-arm failure, `enqueue_accept(server) catch schedule_accept_retry(server)` — a 250 ms `WaitTimer` that re-arms the accept watcher (and re-pauses+retries if fds are still exhausted). On `EMFILE`/`ENFILE`, set the pause flag **and** `schedule_accept_retry(server)` and `return` (void, not an error) so the loop keeps retrying instead of surfacing a callback exception that terminates the loop.
- **Lesson:** A watcher that the liveness of the whole server depends on must have a self-healing retry path. Swallowing re-arm errors with `catch {}` is the same class of bug as "silent failure" (Lesson 57): the listener dies quietly and the failure is nearly impossible to diagnose in production. When in doubt, schedule a bounded retry rather than dropping the watcher.
