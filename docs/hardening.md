...
**Implementation:**
- Evaluated during ring init in [main.zig](file:///home/cwt/Projects/talyn/src/loop/scheduling/io/main.zig#L437).
...
- **Ring fd:** Verified `O_CLOEXEC` is set via `fcntl` in [main.zig](file:///home/cwt/Projects/talyn/src/loop/scheduling/io/main.zig#L479).
- **Eventfd:** Created with `EFD_CLOEXEC` in [main.zig](file:///home/cwt/Projects/talyn/src/loop/scheduling/io/main.zig#L481).
- **Inotify fd:** Created with `IN_CLOEXEC` in [fs_watcher.zig](file:///home/cwt/Projects/talyn/src/loop/fs_watcher.zig#L45).
- **signalfd:** ⚠️ **Fixed.** Changed flag argument in [unix_signals.zig](file:///home/cwt/Projects/talyn/src/loop/unix_signals.zig#L224) from `0` to `std.os.linux.SFD.CLOEXEC`.
- **pidfd:** ⚠️ **Fixed.** Passing `O_CLOEXEC` to the `pidfd_open` syscall is rejected with `EINVAL` by the kernel (which only accepts `PIDFD_NONBLOCK`). We fixed this by opening the pidfd with `0` flags and immediately applying `fcntl(fd, F_SETFD, FD_CLOEXEC)` in [child_watcher.zig](file:///home/cwt/Projects/talyn/src/loop/child_watcher.zig#L53) and [transport.zig](file:///home/cwt/Projects/talyn/src/transports/subprocess/transport.zig#L377).
...
- We added a `@atomicStore` sentinel setting `buffer_memory.ptr` to `0xDEADBEEF` after deinit in [main.zig](file:///home/cwt/Projects/talyn/src/loop/scheduling/io/main.zig#L658) so any stale references trap immediately.
...
Added bounds assertions in `register_fixed_file` and `unregister_fixed_file` in [main.zig](file:///home/cwt/Projects/talyn/src/loop/scheduling/io/main.zig#L539) to check the slot index against `fixed_file_table.len` (1024 slots) before calling `register_files_update`. This prevents undefined behavior (UB) in `ReleaseFast` builds in case of free-list corruption.
...
- **DNS Resolver:** ⚠️ **Fixed.** Modified [resolv.zig](file:///home/cwt/Projects/talyn/src/loop/dns/resolv.zig#L114) to queue `CancelByFd` on the active socket before closing it. Also fixed a memory leak on queue error in `resolv.zig:L795` by slicing `queries_data` to `queries_sent`.
- **FSWatcher:** ⚠️ **Fixed.** Modified `deinit()` in [fs_watcher.zig](file:///home/cwt/Projects/talyn/src/loop/fs_watcher.zig#L28) to explicitly cancel `inotify_task_id` before closing the inotify fd.
