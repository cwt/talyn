...
The Python AsyncIO test suite is **brutal**. We spent a long time fixing bug after bug. This phase became the foundation of [docs/development/lessons/index.md](development/lessons/index.md).

During this period, we hit one extremely stubborn “perma-bug” related to SSL/TLS and the `test_streams` suite ([Priority 20](development/priorities/20-tls-ssl-completion-2026-05.md)). It caused repeated crashes, segfaults, and hangs. For days and weeks, none of my AI models could solve it.
...
After the release of **v0.6.3**, we took a step back and instructed our coding agents to conduct a deep, comprehensive audit of the entire codebase. The goal was to identify and fix any patterns that violated our accumulated [lessons/index.md](development/lessons/index.md). 
