...
pub fn stream_traverse(self: ?*StreamTransportObject, visit: python_c.visitproc, arg: ?*anyopaque) callconv(.c) c_int {
    ...
    // BUG-330: Only visit Python objects that are genuinely owned by the transports.
    // parent_transport is a borrowed backpointer to `instance` itself (visiting it
    // makes Python GC miscalculate reference counts and prematurely collect live
    // transports), and exception_handler is borrowed from loop.exception_handler.
