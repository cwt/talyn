...
inline fn z_loop_subprocess_exec(self: *LoopObject, args: []?PyObject, knames: ?PyObject) !*FutureObject {
    ...
    defer python_c.py_xdecref(py_pid);
...
pub fn loop_subprocess_exec(self: ?*LoopObject, args: ?[*]?PyObject, nargs: isize, knames: ?PyObject) callconv(.c) ?*FutureObject {
    return utils.execute_zig_function(
        z_loop_subprocess_exec,
