...
inline fn z_loop_create_task(self: *PythonLoopObject, args: []?PyObject, knames: ?PyObject) !*PythonTaskObject {
    if (args.len != 1) {
    ...
    if (self.task_factory) |factory| {
        ...
        if (context_passed or name != null) {
            ...
            if (name) |n| {
                _ = python_c.PyDict_SetItemString(py_kwargs, "name\x00", n);
            }
...
pub fn loop_create_task(self: ?*PythonLoopObject, args: ?[*]?PyObject, nargs: isize, knames: ?PyObject) callconv(.c) ?PyObject {
    return @ptrCast(utils.execute_zig_function(z_loop_create_task, .{ self.?, args.?[0..@as(usize, @intCast(nargs))], knames }));
}
