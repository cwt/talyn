...
fn unix_connect_callback(data: *const CallbackManager.CallbackData) !void {
    const ucd: *UnixConnectData = @ptrCast(@alignCast(data.user_data.?));
    ...
    const ret = python_c.PyObject_CallOneArg(connection_made, @ptrCast(transport)) orelse return set_future_exception(error.PythonError, ucd.future);
    python_c.py_decref(ret);

    const result_tuple = python_c.PyTuple_Pack(2, @as(PyObject, @ptrCast(transport)), protocol) orelse return set_future_exception(error.PythonError, ucd.future);
...
inline fn z_loop_create_unix_connection(self: *LoopObject, args: []?PyObject, knames: ?PyObject) !*FutureObject {
...
pub fn loop_create_unix_connection(self: ?*LoopObject, args: ?[*]?PyObject, nargs: isize, knames: ?PyObject) callconv(.c) ?*FutureObject {
    return utils.execute_zig_function(
        z_loop_create_unix_connection,
...
inline fn z_loop_create_unix_server(self: *LoopObject, args: []?PyObject, knames: ?PyObject) !*FutureObject {
    if (Loop.Python.check_forked(self)) return error.PythonError;
    if (args.len < 2) {
        python_c.raise_python_value_error("protocol_factory and path are required");
        return error.PythonError;
    ...
    const py_family_obj = python_c.PyLong_FromLong(std.posix.AF.UNIX) orelse return error.PythonError;
    ...
    const server = python_c.PyObject_CallFunction(
        ...
        "OOOOO\x00",
        @as(*python_c.PyObject, @ptrCast(self)),
        protocol_factory,
        py_fd,
        py_family_obj,
        py_backlog_obj,
...
pub fn loop_create_unix_server(self: ?*LoopObject, args: ?[*]?PyObject, nargs: isize, knames: ?PyObject) callconv(.c) ?*FutureObject {
    return utils.execute_zig_function(
        z_loop_create_unix_server,
        .{ self.?, args.?[0..@as(usize, @intCast(nargs))], knames },
    );
}
